...
SELECT	 order_id, product_id, units,
		 DENSE_RANK() OVER(PARTITION BY order_id ORDER BY units DESC) AS product_rank			-- dense; do not skip rank
FROM 	 orders
...
SELECT 	 customer_id, order_id, MIN(transaction_id) AS min_tid, SUM(units) AS total_units	-- MIN(transaction_id) identifies the earliest transaction for each order, determining which order came first
FROM 	 orders
GROUP BY customer_id, order_id
ORDER BY customer_id, min_tid;								-- ORDER BY min_tid ensures the chronological sequence of orders is preserved
...
SELECT 	customer_id, order_id, total_units,
		LAG(total_units) OVER(PARTITION BY customer_id ORDER BY min_tid) AS prior_units		-- ORDER BY min_tid ensures the chronological sequence of orders is preserved
FROM	my_cte;																				-- tid is not necessary in the table; for an accurate sequence for prior_units column
...
-- final query; using multiple CTEs instead total_units - LAG(total_units) OVER(PARTITION BY customer_id ORDER BY min_tid) **readibility
WITH my_cte 	AS (SELECT 	 customer_id, order_id, MIN(transaction_id) AS min_tid, SUM(units) AS total_units
					FROM 	 orders
					GROUP BY customer_id, order_id
					ORDER BY customer_id, min_tid),
